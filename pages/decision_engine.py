"""
AgroTech Agro Decision Engine Page
Central dashboard synthesizing soil parameters, micro-weather, plant disease status,
and Mandi market prices into a Personalised Farm Action Plan with score breakdown and text download.
"""
...
# Check for session state disease integration from Disease Detection Page
session_disease = st.session_state.get("disease_prediction", None)
if session_disease:
    st.info(f"🌿 **Detected Disease Imported from Diagnostics Page**: `{session_disease}`")
...
with col_soil:
    ...
    n_val = st.slider("Nitrogen (N)", 0, 140, st.session_state.get("de_n", 50), help="Nitrogen ratio in soil (mg/kg)")
    p_val = st.slider("Phosphorus (P)", 0, 145, st.session_state.get("de_p", 50), help="Phosphorus ratio in soil (mg/kg)")
    k_val = st.slider("Potassium (K)", 0, 205, st.session_state.get("de_k", 50), help="Potassium ratio in soil (mg/kg)")
    ph_val = st.slider("Soil pH Level", 0.0, 14.0, st.session_state.get("de_ph", 6.5), step=0.1, help="Soil acidity/alkalinity scale")

with col_weather:
    st.markdown("### 🌦 2. Climate & Micro-Weather")
    temp_val = st.slider("Temperature (°C)", 0.0, 50.0, st.session_state.get("de_temp", 28.0), step=0.5, help="Ambient field temperature (°C)")
    humidity_val = st.slider("Humidity (%)", 0, 100, st.session_state.get("de_hum", 75), help="Relative humidity percentage")
    rainfall_val = st.slider("Rainfall (mm)", 0.0, 300.0, st.session_state.get("de_rain", 120.0), step=5.0, help="Seasonal or forecast rainfall (mm)")
...
with col_crop_mandi:
    ...
    selected_crop = st.selectbox(
        ...
        index=0,
        help="Select crop cultivated or intended for harvest"
    )
    selected_state = st.selectbox(
        "Target State",
        ["Maharashtra", "Punjab", "Uttar Pradesh", "Karnataka", "Gujarat", "Haryana", "Madhya Pradesh"],
        index=0,
        help="Select state to fetch regional Mandi market pricing"
    )
    ...
    disease_option = st.radio(
        ...
        index=0 if not session_disease else 2,
        help="Diagnose from leaf photo, select field outbreak, or mark as healthy"
    )
    ...
    if disease_option == "Upload Leaf Image":
        uploaded_file = st.file_uploader("Upload Leaf Photo", type=["jpg", "jpeg", "png"], help="Upload plant leaf photo for Keras AI diagnosis")
        if uploaded_file:
            ...
            try:
                ...
                if os.path.exists(model_path):
                    with st.spinner("Running deep learning leaf disease diagnosis..."):
                    ...
                    st.info("ℹ️ Using Heuristic AI Disease Severity Assessment.")
            except Exception:
                disease_detected_name = "Fungal Leaf Spot (Detected)"
                disease_severity = "Moderate"
    elif disease_option == "Known Disease Outbreak":
        default_outbreak = session_disease if session_disease else "Moderate Outbreak"
        disease_severity = st.selectbox("Outbreak Severity", ["Moderate Outbreak", "Severe Pathogen Outbreak"], help="Select field outbreak level")
        disease_detected_name = f"Field Outbreak ({disease_severity})"

# Save state
st.session_state["de_n"] = n_val
st.session_state["de_p"] = p_val
st.session_state["de_k"] = k_val
st.session_state["de_ph"] = ph_val
st.session_state["de_temp"] = temp_val
st.session_state["de_hum"] = humidity_val
st.session_state["de_rain"] = rainfall_val

# Trigger Button
st.write("")
generate_plan_btn = st.button("🚀 Generate Personalised Farm Action Plan", type="primary", use_container_width=True)

# --- DECISION ENGINE LOGIC ---
if generate_plan_btn or st.session_state.get("plan_generated", True):
    ...
    # Display Health Score Banner & Breakdown
    ...
    with c1:
        ...
        with st.expander("📈 View Score Breakdown"):
            st.write(f"• **Soil Health Score**: `{int(soil_score)} / 35 pts`")
            st.write(f"• **Weather Risk Score**: `{int(weather_score)} / 35 pts`")
            st.write(f"• **Plant Disease Health**: `{int(disease_score)} / 30 pts`")
    ...
    with c2:
        ...
        if disease_severity == "None":
            ...
            st.error(f"⚠ **Disease Alert**: Severe plant disease outbreak detected ({disease_detected_name}). Immediate spray required!")
    ...
    # Priority 1: Severe Disease Outbreak Control
    if disease_severity != "None":
        if "Late Blight" in disease_detected_name or "Fungal" in disease_detected_name or humidity_val > 80:
            recommendations.append("🧪 **Immediate Fungicide Treatment**: Apply Copper Oxychloride (3g/L) or Mancozeb spray within 12-24 hours to halt foliar spore multiplication.")
        else:
    ...
    # Priority 2: Irrigation & Drainage
    if rainfall_val > 150 or humidity_val > 85:
        recommendations.append("💧 **Avoid Irrigation & Clear Drainage Channels**: Open field drainage immediately. Excess field water retention combined with high atmospheric humidity drastically increases root rot risk.")
    ...
    # Priority 3: Preventative Bio-pesticide if healthy
    if disease_severity == "None":
        recommendations.append("🛡 **Preventative Bio-Protection**: Spray prophylactic Neem oil solution (3ml/L) as a protective barrier against aphid and whitefly vectors.")

    # Priority 4: Soil Nutrient & pH Management
    if ph_val < 6.0:
        ...
        recommendations.append(f"🌱 **Balanced Soil Fertility**: Soil pH ({ph_val}) is ideal. Maintain NPK ratio ({n_val}:{p_val}:{k_val}) with balanced organic bio-fertilizer application.")

    # Priority 5: Market & Selling Advisory
    recommendations.append(f"💰 **Harvest & Mandi Advisory**: Monitor daily modal price trends for {selected_crop.capitalize()} in {selected_state} Mandis. Book transport if market prices remain favorable.")
    ...
    st.write("")
    plan_text = f"AGROTECH PERSONALISED FARM ACTION PLAN\nTarget Crop: {selected_crop.capitalize()} ({selected_state})\nFarm Health Score: {total_health_score}/100\nAssessed Disease: {disease_detected_name}\n\nACTION RECOMMENDATIONS:\n" + "\n".join([f"{i}. {r.replace('**', '')}" for i, r in enumerate(recommendations, 1)])
    
    st.download_button(
        label="📥 Download Action Plan (.txt)",
        data=plan_text,
        file_name=f"AgroTech_Action_Plan_{selected_crop}.txt",
        mime="text/plain"
    )

    # --- SUMMARY OF DATA USED ---
    st.write("")
    with st.expander("🔍 View Decision Engine Processed Data Summary"):
